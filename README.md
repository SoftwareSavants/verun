...
- **Clone from GitHub** - from the first-screen empty state ("Clone from GitHub") or the sidebar `+` menu ("Clone GitHub repo..."); same dialog with a typeahead over your repos (via your existing `gh` CLI auth), a destination folder prompt, `git clone`, and project registration — no terminal hop. Falls back to a paste-a-Git-URL input when `gh` is missing or unauthenticated
