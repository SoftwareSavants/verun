...
- Drag-and-drop now works in the Claude Code terminal view: drop one or more files anywhere over the terminal area and the absolute paths are typed into the PTY (single-quoted with POSIX escaping for spaces and quotes), so Claude's TUI picks them up the same way it would from a native terminal. Hit-tests against the terminal bbox so drops on other panes still go to their owners
