...
- Move the UI / Terminal toggle into the Claude session tab as a tiny 2-segment icon control next to the cost (chat icon for UI, terminal icon for Terminal). Both segments are always visible so it reads as a toggle at a glance, with the active mode highlighted in accent. Tooltips are now jargon-free ("UI view" / "Terminal view"). The previous full-row pill above the chat is gone. Only renders for Claude sessions with a resumable id, and only on the *active* session tab - clicking it on a background tab would silently change a view the user can't see, so the control hides there to avoid the dead-click confusion
