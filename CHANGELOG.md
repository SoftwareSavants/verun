...
- RPC turn failures now surface the JSON-RPC error `data` payload, not just the generic `message` - e.g. Grok's `Internal error` now reads `Internal error: API error (status 402 Payment Required): Grok Build usage balance exhausted`
- Grok models are discovered dynamically via `grok models` (static fallback `grok-4.6` / `grok-4.5`); Grok 1.0 moved the default to `grok-4.6`
- Grok CLI support via ACP (`grok agent stdio`): streamed reasoning and text, tool cards with per-card diffs, interactive permission approvals, resume, and usage/cost. The Codex app-server RPC path was generalized into a protocol-agnostic `Agent::rpc_*` seam (`spawn_rpc_session` + a shared JSON-RPC transport in `agent/rpc.rs`) that both Codex and Grok flow through, so no parallel path was cloned. Plan mode, reasoning-effort selection, and attachments are deferred for Grok (not exposed over ACP yet)
...
- Clone GitHub repo dialog: don't re-fetch the picked repo from GitHub after selecting it — the search input is stamped to the repo URL but the debounced remote lookup short-circuits when the query equals the picked repo's URL or slug. Selected row now uses the same surface as hover (no accent ring), keeping the check icon as the sole "this one's picked" affordance
- Fix `Clone failed: Failed to parse gh repo view output: missing field starCount`. `gh repo view --json` emits `stargazerCount`, not `starCount`; `RemoteRepo.star_count` now accepts both via `#[serde(default, alias = "stargazerCount")]`
- Clone GitHub repo dialog: the inline spinner inside the search input is gone, replaced with shimmer rows in the result list while remote search is in flight; local filter is tokenized and ranked (exact repo-name > prefix > substring > owner > description) so multi-word queries match and best hits surface first
...
- File tree directory listing uses `read_dir` instead of the ignore walker so `.gitignore` / `.ignore` rules no longer hide entries (ignored items stay visible and dimmed only)
- File tree rows for gitignored paths use reduced opacity (`git check-ignore` batched per directory listing), matching the usual editor convention for ignored vs tracked files
...
- Skip the "Do you trust this folder?" Claude TUI prompt for new task worktrees. Verun creates the worktree, picks the branch, and is the one running `claude` in it - the prompt is pure friction in our context. Before spawning the PTY we merge `projects[<worktree>].hasTrustDialogAccepted = true` and `hasCompletedProjectOnboarding = true` into `~/.claude.json` (atomic temp + rename, mutex-serialized to avoid clobbering concurrent spawns, idempotent so re-opens are no-ops, refuses to overwrite a corrupt config so user settings stay safe). Claude has no CLI flag for this; the JSON merge is the only path
- Slash commands run in terminal mode now show in the UI chat as a clean `/<name> <args>` line (e.g. `/adapt wassup`) instead of being dropped or leaking as raw XML. Skill bodies and bare `<local-command-stdout>` post-execution envelopes are still hidden since the model needs them but the user doesn't.
- Filter Claude Code's local-command envelopes (`<local-command-caveat>`, `<command-name>`, `<local-command-stdout>`, etc.) out of the UI chat. The TUI injects them as fake `user` messages so its model can see what slash commands the human ran in terminal mode (`/model`, `/exit`, `/clear`, ...) - they're scaffolding for the model, not user-facing content. They were leaking into UI mode as raw XML + ANSI escapes whenever the user toggled back. Filter at the JSONL parse layer (matches at start of message text only, so real user messages that happen to discuss the syntax still render)
- Terminal-mode Claude sessions now honor the per-session model pick. The spawn command threads `sessions.model` through as `claude --model <name>` (alias like "sonnet"/"opus" or full id like "claude-sonnet-4-6"), matching the SDK-backed UI mode. Empty/unset model falls back to the user's Claude default
- Centered "Starting Claude Code…" spinner now overlays the terminal between the moment the PTY is requested and Claude printing its first frame, so the user gets feedback during the boot gap instead of seeing an empty xterm. Hides on the first non-empty `pty-output` event matching the terminal id (ignores empty keepalives and unrelated terminals)
- Toggling a fresh Claude session to terminal mode now spawns the TUI immediately instead of leaving the user in chat mode. Backend spawns plain `claude` (no `--resume` / no `--session-id`, since `--session-id` is actually a resume-an-existing-session flag despite the misleading help text) and watches `~/.claude/projects/<encoded-cwd>/` for the new `<uuid>.jsonl` Claude writes when the user sends their first turn. On discovery the watcher persists the UUID as the session's `resume_session_id`, emits `session-resume-id` (so the rest of the app instantly sees the session as resumable), and starts the normal transcript tail. Subsequent reopens use the existing `--resume` path. `canUseTerminalView` no longer requires a resume id - any Claude session is eligible
- Move the UI / Terminal toggle into the Claude session tab as a tiny 2-segment icon control next to the cost (chat icon for UI, terminal icon for Terminal). Both segments are always visible so it reads as a toggle at a glance, with the active mode highlighted in accent. Tooltips are now jargon-free ("UI view" / "Terminal view"). The previous full-row pill above the chat is gone. Renders for any Claude session - including fresh ones with no resume id yet, so a brand-new task can opt in to terminal mode before the first message is sent (the actual swap to the PTY is gated by `canUseTerminalView` in TaskPanel and only happens once the resume id arrives, but the user's intent is captured immediately). On inactive session tabs the control collapses to zero width via a 200ms ease-out transition (max-width + opacity, with `-ml-1.5` cancelling the parent's gap so there's no phantom space) so changing the selected session slides the tab width smoothly instead of snapping. Respects `prefers-reduced-motion`. Clicking the toggle is a per-session override only - the global default is changed exclusively via Settings → General → Claude Code, so a one-off toggle on a single session doesn't quietly retrain what every future session opens as
- Switching between two terminal-mode Claude sessions in the same task now actually shows each session's own PTY instead of leaving the visible terminal stuck on whichever one mounted first. The view re-keys on `selectedSessionId` so it remounts on switch. The PTY itself stays alive across switches and across UI ↔ Terminal toggles - `claude_terminal_open` is idempotent per session so the second mount rejoins the existing handle, and ShellTerminal reattaches the existing xterm via the terminals registry. Real cleanup still happens at session close (`close_all_for_task`) and on PTY exit (`drop_if_stale`)
...
- Changes pane: full overhaul. Three sections (Conflicts / Staged Changes / Changes), inline stage/unstage/discard/open-file actions, commit composer with smart-commit, amend, and Commit & Push, conflict-resolve dialog (ours / theirs / stage as-is), and corrected status taxonomy across Rust and TypeScript. Optimistic section-membership patches keep interactions snappy; the watcher reconciles. Preserves the commit draft on failed commit; binary staged/unstaged diffs short-circuit instead of rendering garbled UTF-8; Stage All and smart-commit fallback refuse when conflict markers are present; Amend is disabled while conflicts exist
...
- MCP server (in-app host): exposes Verun task state to Claude Code via JSON-RPC 2.0 over a Unix socket at `<app_data>/mcp.sock`. First slice ships `verun_list_tasks` (pagination + scope filter); per-connection identity handshake (`{"task_id":"..."}`) tells the host which task is calling so default-scope queries return only sibling tasks within the caller's project. Spec version `2025-11-25`. Unix-only feature (macOS + Linux); Windows builds skip the in-app host and the relay binary is a no-op stub
- New `verun-mcp-relay` binary: thin per-session stdio<->Unix-socket forwarder Claude Code launches as its MCP server. Reads `VERUN_TASK_ID` + `VERUN_MCP_SOCKET` from env, writes the identity frame, then ndjson-pipes between stdin/stdout and the in-app host. Stdin EOF half-closes the socket so in-flight responses still drain
- `create_task` writes `<worktree>/.mcp.json` so Claude Code auto-discovers the Verun MCP server on launch — `command` points at the sibling `verun-mcp-relay` binary, `env` carries `VERUN_TASK_ID` and the socket path. Existing `.mcp.json` files (e.g. project-level entries committed to the repo) are merged: only the `verun` entry is overwritten, every other server survives. Failure is non-fatal; task creation continues and the task just runs without the in-app MCP. Skipped on Windows (Unix domain sockets only)
- `verun_read_task_output` MCP tool: read the most recent agent output from a sibling task. Tail by byte budget (default 16 KiB, 1 KiB-64 KiB clamp) with cursor-based pagination for older history; defaults to the latest session for the task, or pass `session_id` to target a specific one (must belong to the named task). Returns chronological lines plus task/session metadata so an agent can reason about what its sibling is doing
- `verun_send_message` MCP tool: deliver a user-style prompt to a sibling task's running agent. Goes to the latest session by default or an explicit `session_id` (cross-task validation rejects mismatches). The tool routes through an in-app action worker that holds the Tauri runtime state, so the message lands via the same path as a UI-typed message - persistent agents reuse their stdin, non-persistent agents respawn. Returns once the message has been queued; agents observe responses via verun_read_task_output
- `verun_spawn_task` MCP tool: create a new sibling task with its own worktree, branch, and first agent session. Defaults to the caller's project, claude agent, and project base branch; `project_id` / `base_branch` / `agent_type` (claude|codex|cursor|gemini|opencode, anything else is rejected) override per-call. Optional `initial_message` is delivered to the new agent immediately after boot so a calling agent can hand off work in a single round-trip. Returns task_id, generated branch, session_id, agent type, and whether the initial_message was delivered
- `verun_app_start` / `verun_app_stop` / `verun_app_logs` MCP tools: control a task's project start command (Dev Server) without leaving the agent. `app_start` is idempotent - if a Dev Server PTY is already alive for the task it returns the existing terminal_id with `already_running=true` instead of spawning a duplicate. `app_stop` returns `stopped=false` when nothing was running so it's safe to call speculatively. `app_logs` tails the start command's buffered terminal output (default 16 KiB, 1 KiB-64 KiB clamp) for boot verification, framework warnings, or post-crash stack traces. All three default to the caller's task and reject unknown task_ids with E_INVALID_PARAMS
- `verun-mcp-relay` is now bundled into `Verun.app/Contents/MacOS/` for production builds via Tauri's `externalBin` sidecar mechanism. A small `scripts/copy-relay-sidecar.mjs` runs in `beforeBuildCommand` to drop an empty placeholder at the path tauri-build validates, then runs again in `beforeBundleCommand` to overwrite it with the cargo-built `target/release/verun-mcp-relay` so the real executable lands in the .app. Release `Verun.app` installs now have a working in-app MCP server out of the box; production parity with the dev workflow
- Pre-approve 6 safe MCP tools (`verun_app_logs`, `verun_list_tasks`, `verun_list_sessions`, `verun_app_start`, `verun_app_stop`, `verun_app_restart`) in `.claude/settings.local.json::permissions.allow` so sibling agents don't prompt on every call. Read-only and worktree-scoped lifecycle operations stay frictionless. Mutating tools (`verun_send_message`, `verun_create_task`, `verun_create_session`) remain gated
- `verun_list_sessions` MCP tool: list a task's sessions so an agent can pick a specific `session_id` for `verun_read_task_output` or `verun_send_message`. Defaults to the caller's task and running sessions only; pass `include_closed=true` for closed ones. Returns a trimmed projection (id, name, status, agent, model, started_at, ended_at, parent_session_id, forked_at_message_uuid, total_cost) without internal token columns
- `verun_create_session` MCP tool: start a fresh agent session inside an existing task (same worktree, same branch) so an agent can spin up a clean conversation thread without losing the task's working state. Defaults to the caller's task and inherits the agent_type of the task's most recent session (falling back to the project default). Optional `model` override; optional `initial_message` is delivered to the new session immediately after creation. Routes through the same action worker as `verun_send_message`/`verun_spawn_task` so it lands on the runtime state correctly
- Helpful errors for session lookups: `verun_read_task_output` and `verun_send_message` now point agents at `verun_list_sessions` when a `session_id` doesn't exist, and at `verun_create_session` when a task has no sessions yet. Cross-task session_id mismatches now name both task IDs and suggest the correct argument to pass
- Fix terminal pane showing a blank dev server when started while terminal is already open. Reactive store updates in `spawnTerminal` are now batched so `ShellTerminal` mounts with the correct active state immediately instead of starting hidden, preventing xterm.js from initializing with 0×0 dimensions
