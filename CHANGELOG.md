...
- Fix stale-glyph artifacts in every xterm-based view (Claude terminal, shell terminals, bootstrap scaffold log) where the layout looked broken after each new message and only cleared on a window resize - resize was papering over WebGL texture-atlas drift by re-blitting every cell. Apply the canonical fix used by VS Code (`xtermTerminal.ts: forceRedraw()`) and Tabby (`xtermFrontend.ts: displayMetricsChanged$`): drop `customGlyphs` + `rescaleOverlappingGlyphs`, install a `WebglAddon.onContextLoss` handler that disposes the renderer (so xterm auto-falls back to DOM on macOS WebKit context drops in Tauri), and call `term.clearTextureAtlas()` on theme/font changes, DPR changes (window dragged between Retina and non-Retina displays), and visibility return (WebKit suspends GL contexts when the window is hidden)
