...
- Toggling a fresh Claude session to terminal mode now spawns the TUI immediately instead of leaving the user in chat mode. The backend pre-generates a UUID and spawns `claude --session-id <uuid>` (an officially-supported flag - "Use a specific session ID for the conversation"); Claude creates the conversation with that id and we persist it as the session's `resume_session_id` and emit `session-resume-id` so the rest of the app sees the session as resumable. Subsequent reopens use the existing `--resume` path. `canUseTerminalView` no longer requires a resume id - any Claude session is eligible
