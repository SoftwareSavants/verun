...
- Move the UI / Terminal toggle into the Claude session tab as a tiny icon button (Terminal / MessageSquare lucide glyph) next to the cost. The previous full-row pill above the chat is gone. The icon shows the *target* mode you'll switch to on click; the button only renders for Claude sessions that have a resumable id
