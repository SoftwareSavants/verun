...
- Fix Claude TUI rendering at the wrong size on first paint until the user manually resized the window: `claudeTerminalOpen` was hardcoded to spawn the PTY at 24x80, so Claude painted its first frame at that size and only redrew on the SIGWINCH that arrived once xterm fitted to the real container. `SessionTerminal` now measures its container with `estimatePtyDimensions` (uses the configured xterm font size) and passes the real rows/cols on open, deferred to a `requestAnimationFrame` so flex layout has computed before measuring
