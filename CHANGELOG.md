...
- Open editors now refresh in real time when the file changes on disk (e.g. Claude Code writes to it), not only when the window regains focus. fileSync subscribes to the watcher's `file-tree-changed` events and silently reloads clean tabs, surfaces a persistent "Conflicting changes" toast on dirty tabs, and flips tabs to a deleted state (red ⊘ + strikethrough) when the file goes away
- `checkBeforeSave` no longer opens the conflict dialog when the user pressed cmd+S without local edits: it silently syncs the editor to disk and skips the no-op write. Fixes the spurious "your changes will override external changes" prompt when nothing was actually changed locally
- Save-time conflict dialog and the dirty-tab toast now share the same wording and three-way action vocabulary: "Conflicting changes" / Cancel / Take disk / Keep mine. The toast surfaces in real time on watcher events instead of waiting for the next window focus
- Cached tabs now re-verify against disk when reopened, so the editor can't silently serve stale content for files edited externally while the tab was closed
- Save on a deleted-state tab opens a Recreate confirmation modal instead of writing blindly. Recreate mounts in both the main window and detached task windows
- Atomic-write robustness: the deletion declaration is debounced ~250ms and re-confirmed against disk, so tmp→rename writes don't briefly flash the deleted-state UI
