...
- Cmd+V now pastes images into the Claude terminal view: a capture-phase keydown handler on `SessionTerminal` runs before `ShellTerminal`'s text-only Cmd+V intercept, calls a new `read_clipboard_image_to_path` IPC (osascript dumps the clipboard image to `<app_data>/paste_tmp/clip-<uuid>.png`), and writes the saved path into the PTY single-quoted. When the clipboard has no image it falls through to the existing text-paste flow, so plain text pastes still work
