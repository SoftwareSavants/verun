...
- Fix stale-glyph artifacts in every xterm-based view (Claude terminal, shell terminals, anything using `ShellTerminal`) where the layout looked broken after each new message and only cleared on a window resize. The combination of `customGlyphs: true` + `rescaleOverlappingGlyphs: true` + `WebglAddon` left stale cells in the GPU buffer when content reflowed mid-session - resizing was papering over it by re-blitting every cell. VS Code hit the same bug and disables both flags by default. Dropped them; WebGL stays for perf
