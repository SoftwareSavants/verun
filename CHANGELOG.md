...
- Fix session cost + token chips regressing to a partial sum after the 250-line cap: cost now trusts the DB-maintained `total_cost` seeded by `loadSessions`, and tokens are seeded via a new `get_session_token_totals` IPC that scans every persisted `turnEnd` item server-side instead of replaying only the visible tail
