...
- Fix Codex turns stalling after `item/tool/requestUserInput` — Verun now auto-replies with `{answers: []}` so the CLI treats it as the user declining to answer and the turn proceeds cleanly; other unrecognized server-originated JSON-RPC requests get a `-32601` method-not-found response instead of silently being ignored (previously the process sat waiting forever)
