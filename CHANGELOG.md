...
- Pre-approve 5 safe MCP tools (`verun_app_logs`, `verun_list_tasks`, `verun_app_start`, `verun_app_stop`, `verun_app_restart`) in `.claude/settings.local.json::permissions.allow` so sibling agents don't prompt on every call. Read-only and worktree-scoped lifecycle operations stay frictionless. Mutating tools (`verun_send_message`, `verun_create_task`) remain gated
