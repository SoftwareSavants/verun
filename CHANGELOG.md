...
- Fix stale-glyph artifacts in every xterm-based view (Claude terminal, shell terminals, bootstrap scaffold log) where the layout looked broken after each new message and only cleared on a window resize. Swapped `WebglAddon` for `@xterm/addon-canvas`: WebGL's texture-atlas drifted under mid-stream content reflow on macOS WebKit (Tauri), leaving stale glyphs at old cell positions until a manual resize re-blitted every cell - the canvas renderer rebuilds the visible grid on every dirty-cell update so it's immune. VS Code uses canvas as its fallback for exactly this reason. Also dropped `customGlyphs` + `rescaleOverlappingGlyphs` (fragile xterm config) and added `clearTextureAtlas` lifecycle hooks for theme/font/DPR/visibility events as defence-in-depth
